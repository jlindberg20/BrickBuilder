...
def _enrich_with_rb_and_colors(res, rbmap, ldraw_colors):
    """
    - Resolve rb_part_num for placements.
    - Aggregate BOM by (rb_part_num, ldraw_color).
    - Write color_rgb_hex when known.
    """
    steps = res["steps"] or []
    ...
    for st in steps:
        for pl in st["placements"]:
            rb = pl.get("rb_part_num")
            if not rb:
                rb = infer_ldraw_stem(pl.get("subfile",""))
                pl["rb_part_num"] = rb
            lcd = pl.get("ldraw_color")  # LDraw color code integer
            key = (rb, lcd)
            bom_counts[key] = bom_counts.get(key, 0) + pl.get("qty", 1)

    # Build BOM rows with hex color (schema field color_rgb_hex)
    bom = []
...
def build_record(ldr_path: str, parsed, master_parts_version: str | None = None):
    ...
    # Placements → schema-compliant (omit None; do not emit ldraw_color)
    steps = parsed["steps"]
        ...
        for st in steps:
            ...
            for pl in st["placements"]:
                base = {
                    ...
                    "rb_color_id": pl.get("rb_color_id"),  # may be omitted if None by _drop_none
                    "qty": pl.get("qty", 1),
    ...
    record = {
        ...
        "type": "model",
        "source_ids": {
            "ldraw": {"root_file": ldr_path, "file_stem": stem}
        },
        "name": stem,
        "metadata": {
        ...
        "geometry": {
            "ldraw": {
                "root_file": ldr_path,
                "scale": {"units": "LDU", "to_mm": LDU_TO_MM}
            }
        },
        "bom": parsed["bom"],
        "steps": out_steps,
        "instructions": {
            "kind": "ldraw",
            "source": ldr_path,
            "parsed_confidence": 1.0 if out_steps else 0.5
        },
        "links": {
            "master_parts_version": master_parts_version or "",
            "parser_version": "ldraw_parser_v0.3",
            "build_tools": "python"
        }
    }
    return record
